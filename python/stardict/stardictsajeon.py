...
def get_full_dict_name(file_name):
    return os.path.join(os.path.dirname(__file__), '..', '..', '..', '..', 'python', 'kreader_dicts', file_name)
...
class StardictBaseSajeon:
    ...
    def get_definition(self, word, add_examples=False):
        ...
        def get_article(index):
            sametypesequence, data = next(iter(dicts[index].items()))
        ...
        if len(dicts) == 1:
            return get_article(0)
        return [get_article(i) for i in range(len(dicts))]
...
class StardictKoSajeon(StardictBaseSajeon):
    def __init__(self):
        dict_dir = get_full_dict_name(r'stardict-KoreanDic-2.4.2')
        dict_name = 'KoreanDic'
        super().__init__(dict_dir, dict_name)
